...
def expected_offspring(input_file):
    with open('output.txt','w') as output, open(input_file) as input:
        parameters = input.read().split()
        parameters = [int(a) for a in parameters]
        probabilities = [1,1,1,0.75,0.5,0]
        result = 0
        for count, probability in zip(parameters, probabilities):
            result += count * 2 * probability
        output.write(str(result))

expected_offspring('input.txt')
